...
def get_possible_moves(tablero):
    tamaño = len(tablero)
    moves = []
    for capa in range(tamaño):
        for fila in range(tamaño):
            for col in range(tamaño):
                if tablero[capa][fila][col] == " ":
                    moves.append((capa, fila, col))
    return moves

def evaluate_move(tablero, move):
    capa, fila, col = move
...
def minimax(tablero, tamaño, depth, alpha, beta, is_maximizing, transposition_table):
    ...
    if is_maximizing:
        max_eval = float('-inf')
        moves = get_possible_moves(tablero)
        moves.sort(key=lambda move: evaluate_move(tablero, move))
        for move in moves:
            capa, x, y = move
            tablero[capa][x][y] = 'O'
        ...
        min_eval = float('inf')
        moves = get_possible_moves(tablero)
        moves.sort(key=lambda move: evaluate_move(tablero, move), reverse=True)
        for move in moves:
            capa, x, y = move
            tablero[capa][x][y] = 'X'
