...
def minimax(tablero, depth, alpha, beta, is_maximizing, jugador, tamaño):
    if depth == 0:
        return evaluate(tablero, tamaño)
...
def realizar_movimiento_bot(tablero, jugador, tamaño):
    ...
    for capa in range(tamaño):
        for fila in range(tamaño):
            for col in range(tamaño):
                if tablero[capa][fila][col] == " ":
                    tablero[capa][fila][col] = jugador
                    score = minimax(tablero, 3, -float('inf'), float('inf'), False, 'X', tamaño)
                    tablero[capa][fila][col] = " "
                    if score > best_score:
                        best_score = score
                        move = (capa, fila, col)
    capa, fila, col = move
    tablero[capa][fila][col] = jugador
This realizar_movimiento_bot function uses the improved minimax function to determine the best move for the bot. It sets a depth limit of 3 and initializes the alpha and beta values to negative and positive infinity, respectively. The function then iterates over all possible moves and uses the minimax function to evaluate each move. The move with the highest score is selected as the best move for the bot and is made on the game board.
